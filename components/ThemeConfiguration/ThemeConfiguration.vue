...
import { sanitizeKey, restructureFontSizeObject, cloneDeep } from './helpers.js';
		...
		extractColorRules(object) {
			...
			Object.entries(object).forEach(([key, value]) => {
				...
				const composedValue = String(value?.value ?? value)
					...
					.join(' ');
				rules.push(
					`--theme-colors-${sanitizeKey(key)}: ${composedValue};`
				);
			});
			...
			['letterSpacing'].forEach((key) => {
				if (object[key]) {
					...
					for (const name in object[key]) {
						...
						rules.push(
							`--theme-letterSpacing-${sanitizeKey(name)}: ${
								subObject.sm
							}em;`
						);
			...
			['lineHeight'].forEach((key) => {
				if (object[key]) {
					...
					for (const name in object[key]) {
						...
						rules.push(
							`--theme-lineHeight-${sanitizeKey(name)}--sm: ${
								subObject.sm
							};`
						);
						rules.push(
							`--theme-lineHeight-${sanitizeKey(name)}--md: ${
								subObject.md
							};`
						);
						rules.push(
							`--theme-lineHeight-${sanitizeKey(name)}--lg: ${
								subObject.lg
							};`
						);

						rules.push(
							`--theme-lineHeight-${sanitizeKey(name)}: ${
								subObject.sm
							};`
						);
						if (subObject.md !== subObject.sm) {
