# Changelog

## [3.3.3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.3.2...theme-configuration-v3.3.3) (2026-02-17)


### Bug Fixes

* added space to trigger build ([7206ee1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7206ee1fca7fc83f8dd5cf701e4390f3539271de))

## [3.3.2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.3.1...theme-configuration-v3.3.2) (2026-02-17)


### Bug Fixes

* some minor cleanup ([09128ea](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/09128ea2048a7ebe677f876a1c728b0671173657))
* try useHeadSafe instead ([4107138](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/41071381287275cc8b0a1f36f6f4477cf0978b52))

## [3.3.1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.3.0...theme-configuration-v3.3.1) (2026-02-16)


### Bug Fixes

* refer headStyles directly in useHead ([9d823fa](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/9d823fab16bc6bc290efc880e24f903d093c3cad))

## [3.3.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.2.2...theme-configuration-v3.3.0) (2026-01-16)


### Features

* improved efficiency ([75d6b5a](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/75d6b5a3670ddae8c3be9fb627e53b045938caed))
* improved efficiency further ([8192a4f](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/8192a4fcc1da697e6de8347e8c8c76cf91ef7066))


### Bug Fixes

* return to earlier state ([26e0544](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/26e0544055d73961e9d77d91bdd33c464b25b93c))

## [3.2.2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.2.1...theme-configuration-v3.2.2) (2026-01-16)


### Bug Fixes

* only use stucturedClone if available ([1ea7d5a](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/1ea7d5aabb83682805ebd7f581adb5a2a90506e7))

## [3.2.1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.2.0...theme-configuration-v3.2.1) (2026-01-16)


### Bug Fixes

* reduce the amount of objects and arrays ([b98ce51](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b98ce51e6b41b3271059c23ac3760bc36dc18b50))

## [3.2.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.1.2...theme-configuration-v3.2.0) (2026-01-16)


### Features

* trying to make everything more efficient and lazy ([8e3cf7b](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/8e3cf7b3c7eb4071fdd3c4069e7057033fa8c319))

## [3.1.2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.1.1...theme-configuration-v3.1.2) (2026-01-16)


### Bug Fixes

* more memory leak avoidance ([68ba6e3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/68ba6e3b95767a47a59e307634c9ad4273a8096b))

## [3.1.1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.1.0...theme-configuration-v3.1.1) (2026-01-15)


### Bug Fixes

* wrap in runWithContext ([c2fea49](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/c2fea499155bf787956ba805394f8333cabc9e9a))

## [3.1.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v3.0.0...theme-configuration-v3.1.0) (2025-12-18)


### Features

* made able to configure extra theme configurations ([33dca40](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/33dca4040b306865188bc673c560cc396bad6d29))


### Bug Fixes

* prefer local ([869421d](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/869421d60a60d5b6b69f7c502cf4e7ddf0248cbe))

## [3.0.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/theme-configuration-v2.8.8...theme-configuration-v3.0.0) (2025-11-10)


### ⚠ BREAKING CHANGES

* color separation is now using spaces instead of commas to work with latest unocss update

### Features

* added extra utility functions ([1732333](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/17323331d7d33ebcf5c90700a7e4c05cd5b69a0f))
* added functionality to make theme classes from the configurations ([8a7e0f5](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/8a7e0f5dc8e86d0a833daa694620cd0bb42a592c))
* added getThemeConfiguraitons util function ([4a7d171](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/4a7d1715aca798b9dee3fdefb17f0aa68298608c))
* added getThemeConfiguration utility function ([6892a47](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/6892a47a889e59ec6078bb25ac4d1b71f62ab65b))
* added posibility for print config ([130c1a7](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/130c1a7803642b425ebc13e2dc0387e5869c90e3))
* added proper lint configs ([acc99da](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/acc99da572e887b66731abbf8ca060f1404535c5))
* added release please ([2cb2e42](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/2cb2e429e1cac55b088e156118fc443f4a4a4dfb))
* added rounding to theme configuration capabilities ([35f4e43](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/35f4e43b699f12ce3d2f6f8e123aa66cc8cfc3e5))
* added support for `fontStyles`, `backgroundColors`, `borderColors` and `textColors`. ([3b3fe39](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/3b3fe3924e6b77f0ee2bda2d5918c145fce68634))
* added workflow ([63c1cf0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/63c1cf06294493a982a37a04e02abf57b7563d61))
* change point of reference inside helpers ([5445c97](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/5445c978b7770e5fd49b314fd1dbc0add8fa837b))
* color separation is now using spaces instead of commas to work with latest unocss update ([1f59686](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/1f596861e810c51af6b85e3858b51db12a4d4eef))
* converted to mjs files ([51c01fb](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/51c01fbfe27dcf9f11447c12cbdbc51a94caf207))
* converting ThemeConfiguration to nuxt3 ([e76f38c](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/e76f38c4984fddefb15cb3e202e63147d58f67dd))
* dont import config in helpers ([fb0bf1a](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/fb0bf1ad62b7d530e8860d7f717df5cc35d9ed00))
* handle quotes around font family ([7ae7663](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7ae7663396323b8b666d17ab16dfbb514be7ccc3))
* made configs per media query possible ([22c2d66](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/22c2d66a03e8139ebcacc67a4cc7ac53393d111e))
* made opacity control work ([21dd3fd](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/21dd3fde7ed439b3c8fc3bb8b5fde86c86cf24b9))
* made some final adjustments to make it wokr ([725c489](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/725c489273ead26d7dbf7ae8e36cd1915f93d411))
* made the using of theme configurations better ([f74d157](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/f74d157669537c766e764e770037624ba481d937))
* made theme config not crash on missing config ([60f7f38](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/60f7f38fa0660e2f75c65841d1c615dfc1222ce1))
* minor tweaks plus added eslint ([1ac059b](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/1ac059b1b9382de51b1c9632d84f2f1ef5fac75c))
* moved styles to useHead ([b3e399e](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b3e399ec5fde109adfe9898b79d4eef26ce3f99e))
* update with unocss ([514182d](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/514182d371571c0b840b33f8c814f755039ca945))
* upgrade of color on background handling ([6a9a20e](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/6a9a20e369f7e0e2e9c93b67e936a9088c45e7bf))
* upgrade to latest ([7c3ac30](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7c3ac304693f018d0d007cd538a3aab71b605745))
* working on the query setup ([6440374](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/64403741e6730311be485c8d9e8b91e150819c4b))


### Bug Fixes

* `themeUtilities` being overwritten in `helpers.uno.js` ([53de797](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/53de797df003608282a08940aa90e059a3d31d6b))
* added deepmerge as a dependency ([80cccdc](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/80cccdca58c0bac2d7accb4b49d151a71e5aa48a))
* added dot to manifest ([302d1d9](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/302d1d98065608916163db9f5f0c7466c406d379))
* added filter boolean to preset fetches ([ee631c2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/ee631c2cabd5d9f0e1e8a1959b4f5461326c1c67))
* added manifest ([66847d3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/66847d3fabeb6d52885949922ce9720c88018526))
* auth token on build ([21ec378](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/21ec3783428c2dbf01f7235e8837714f063bb090))
* await load initial themes ([40ed74a](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/40ed74ae4e991bcb038490657756bb2b918b803a))
* border & text color bug ([e7e2f15](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/e7e2f1566f929c9458bea398f5a8747279343a7d))
* border & text color bug ([d1a9190](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/d1a919042fbbfc6b4303e7e5f685117baddd61b7))
* change nuxt config to ts ([4db3085](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/4db3085a5c223c32cc6121a91ba4a255a9c6ecce))
* change nuxt config to ts ([35c4e4f](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/35c4e4f18a05e06a58f396c24d7e3422b6ade605))
* changed reference to clone-deep ([4dc0ada](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/4dc0ada20fd842833391a28d80868758e54c4efb))
* changed to export default ([b7db1e4](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b7db1e409d4e191b344caf02c2f8338d041a88ea))
* default as ([ddf1177](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/ddf117713b1d27ec3c5c8310227dee9e275af6c0))
* default column rules to dvw ([34cc6af](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/34cc6aff27cdc9d8800c0d20be913c7c2f65a659))
* default module handling ([b2a8620](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b2a8620c2cb108633eab463f8f3a4f4c0fd9caa7))
* doing my own deep cloning then ([4436dab](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/4436dab9b9b636db4d36f2142b6dae157b3e779e))
* dynamic head styles ([b102dc7](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b102dc7375b93b9185f8d450c9511b805469414e))
* expanded regexp for config names ([6e8e8e2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/6e8e8e2329a13368b02b1b8c6c8c8ba99d71a4d0))
* expanded relase-package ([c0457fb](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/c0457fb776482097b9a8081cde823a98d3cfc6cc))
* globs back into async function ([72dd858](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/72dd8586473b928e15249326daa318ea34f4bb6b))
* improved handling of theme configurations ([6dd3c47](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/6dd3c47080b207a71dc2c8983249ea72e9d32ea0))
* linted everything ([06cec36](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/06cec36561b9117d23e9171ce39c265463e924ce))
* made config keys count up on changes ([bad78bb](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/bad78bbed34ec5ec1a066cea8f2d5edf08bcdc51))
* memory leak problem ([7712c56](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7712c56e0d3a1646587f028afa66fc3b8aa6483e))
* memory leak problem ([b7bad40](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b7bad4025bd65b57feb9900adf7b4bb6855045e6))
* more versatile functions ([57fb4aa](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/57fb4aa89147150d1829066154e8ff376cb02fba))
* move glob out of function ([e565050](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/e565050914fa5ffbba365251ed73cb4ae45584ed))
* moved load to component ([e555905](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/e555905fec010c5d2cf07164b2eb205519e53e44))
* moved the use of async function ([88af97f](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/88af97f8b3e09d53fb0f4138fe8cb8a3965a531a))
* naming should be right ([845a007](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/845a007a4c531d89d3abb09dd36cd33325b866ad))
* nope ([d95e18b](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/d95e18b91a9c93fe76c15629102b3b46f51773d8))
* not await on the import ([fd790f7](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/fd790f73d53b917a8200c4b662eeea7575202731))
* npmrc auth token ([af75cdd](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/af75cdd3eddbd562e2ef8c72afcce113532749a7))
* out again ([16d8469](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/16d84691cb5258e0aa980f59a01538b781689370))
* read packages ([eeed40d](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/eeed40d731bcca8aea5c1227151168b5bb9116c8))
* removed duplicate ([da5c955](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/da5c95508b41e7c7283efc5dd5f9d063030115d3))
* removed nuxt-core package ([edb24ec](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/edb24ec53a11071b88f37828a010c92f4611abb3))
* removed required as it doesn't work ([db003d4](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/db003d44ff43609849b36f8ad3e9d89de4887a46))
* replaced deepmerge ([46afb49](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/46afb49f5ad74cc9ee84c3656ecace4389bc3c94))
* replaced deepmerge ([1acc921](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/1acc921422f6687e4076099535bec0bbc725a3a9))
* surround font family with quotes when containing special characters ([8d6b1e0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/8d6b1e0c6b2dbcf60acb8f6d2018a8cb2a7af98b))
* try again ([980a286](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/980a286d0a1bfa607962ce533d0b4600ce6f3c8c))
* try again ([b11704e](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b11704e9ebb2c1753620d7ff0d2a3941be36a45d))
* ts in package as well ([7e76468](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7e764685db0501f72e480fa834b9d63d2df71d06))
* typo in `ThemeConfiguration` ([b6564e3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b6564e31c7b026d0e715fa496f66c676d0c808d4))
* updated lock file ([1a431fd](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/1a431fd3e9f5829de00a8d9d13f6f623b37fcf67))
* updated release please ([c9dfd78](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/c9dfd7862df4e96768d02cd7cf06e1c3cd2b22ac))
* updated release please node version ([7ec37d3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7ec37d3349b9e9e98856605d96f853944ec96bc8))

## [2.8.8](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.7...v2.8.8) (2025-11-10)


### Bug Fixes

* updated lock file ([1a431fd](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/1a431fd3e9f5829de00a8d9d13f6f623b37fcf67))

## [2.8.7](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.6...v2.8.7) (2025-11-10)


### Bug Fixes

* try again ([980a286](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/980a286d0a1bfa607962ce533d0b4600ce6f3c8c))

## [2.8.6](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.5...v2.8.6) (2025-11-10)


### Bug Fixes

* updated release please node version ([7ec37d3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7ec37d3349b9e9e98856605d96f853944ec96bc8))

## [2.8.5](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.4...v2.8.5) (2025-11-10)


### Bug Fixes

* try again ([b11704e](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b11704e9ebb2c1753620d7ff0d2a3941be36a45d))

## [2.8.4](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.3...v2.8.4) (2025-11-10)


### Bug Fixes

* added filter boolean to preset fetches ([ee631c2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/ee631c2cabd5d9f0e1e8a1959b4f5461326c1c67))

## [2.8.3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.2...v2.8.3) (2025-10-21)


### Bug Fixes

* improved handling of theme configurations ([6dd3c47](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/6dd3c47080b207a71dc2c8983249ea72e9d32ea0))

## [2.8.2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.1...v2.8.2) (2025-10-02)


### Bug Fixes

* more versatile functions ([57fb4aa](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/57fb4aa89147150d1829066154e8ff376cb02fba))

## [2.8.1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.8.0...v2.8.1) (2025-10-02)


### Bug Fixes

* ts in package as well ([7e76468](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7e764685db0501f72e480fa834b9d63d2df71d06))

## [2.8.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.7.4...v2.8.0) (2025-10-02)


### Features

* added extra utility functions ([1732333](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/17323331d7d33ebcf5c90700a7e4c05cd5b69a0f))


### Bug Fixes

* naming should be right ([845a007](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/845a007a4c531d89d3abb09dd36cd33325b866ad))

## [2.7.4](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.7.3...v2.7.4) (2025-10-02)


### Bug Fixes

* change nuxt config to ts ([35c4e4f](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/35c4e4f18a05e06a58f396c24d7e3422b6ade605))

## [2.7.3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.7.2...v2.7.3) (2025-10-02)


### Bug Fixes

* memory leak problem ([b7bad40](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b7bad4025bd65b57feb9900adf7b4bb6855045e6))

## [2.7.2](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.7.1...v2.7.2) (2025-05-21)


### Bug Fixes

* made config keys count up on changes ([bad78bb](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/bad78bbed34ec5ec1a066cea8f2d5edf08bcdc51))

## [2.7.1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.7.0...v2.7.1) (2025-05-14)


### Bug Fixes

* dynamic head styles ([b102dc7](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b102dc7375b93b9185f8d450c9511b805469414e))

## [2.7.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.6.0...v2.7.0) (2025-05-07)


### Features

* moved styles to useHead ([b3e399e](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b3e399ec5fde109adfe9898b79d4eef26ce3f99e))

## [2.6.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.5.1...v2.6.0) (2024-09-23)


### Features

* upgrade to latest ([7c3ac30](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7c3ac304693f018d0d007cd538a3aab71b605745))


### Bug Fixes

* linted everything ([06cec36](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/06cec36561b9117d23e9171ce39c265463e924ce))

## [2.5.1](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.5.0...v2.5.1) (2024-09-05)

### Bug Fixes

-   surround font family with quotes when containing special characters ([8d6b1e0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/8d6b1e0c6b2dbcf60acb8f6d2018a8cb2a7af98b))
-   typo in `ThemeConfiguration` ([b6564e3](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/b6564e31c7b026d0e715fa496f66c676d0c808d4))

## [2.5.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.4.0...v2.5.0) (2024-06-04)

### Features

-   added rounding to theme configuration capabilities ([35f4e43](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/35f4e43b699f12ce3d2f6f8e123aa66cc8cfc3e5))

## [2.4.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.3.0...v2.4.0) (2024-03-28)

### Features

-   added getThemeConfiguration utility function ([6892a47](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/6892a47a889e59ec6078bb25ac4d1b71f62ab65b))

## [2.3.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.2.0...v2.3.0) (2024-03-07)

### Features

-   handle quotes around font family ([7ae7663](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/7ae7663396323b8b666d17ab16dfbb514be7ccc3))
-   working on the query setup ([6440374](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/64403741e6730311be485c8d9e8b91e150819c4b))

## [2.2.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/v2.1.0...v2.2.0) (2024-01-31)

### Features

-   made configs per media query possible ([22c2d66](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/22c2d66a03e8139ebcacc67a4cc7ac53393d111e))

## [2.1.0](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/compare/2.0.1...v2.1.0) (2024-01-30)

### Features

-   added getThemeConfiguraitons util function ([4a7d171](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/4a7d1715aca798b9dee3fdefb17f0aa68298608c))
-   added posibility for print config ([130c1a7](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/130c1a7803642b425ebc13e2dc0387e5869c90e3))
-   added release please ([2cb2e42](https://github.com/limbo-works/Limbo.Nuxt.ThemeConfiguration/commit/2cb2e429e1cac55b088e156118fc443f4a4a4dfb))
